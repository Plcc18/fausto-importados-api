...
public class UserController {
    ...
    public ResponseEntity<ApiResponse> postUser(@Valid @RequestBody User u) {
       if (userService.existByEmail(u.getEmail())) {
           return ResponseEntity.status(HttpStatus.CONFLICT)
                   .body(new ApiResponse(Instant.now().toString(), "User already registered"));
       }

       u.setPassword(passwordEncoder.encode(u.getPassword()));
       userService.save(u);

       return ResponseEntity.ok(new ApiResponse(Instant.now().toString(), "User created successfully"));
    }

    //Put
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse> putUser(@PathVariable UUID id, @Valid @RequestBody User u) {
        ...
        return ResponseEntity.ok(new ApiResponse(Instant.now().toString(), "User updated successfully"));
    }
    ...
    public ResponseEntity<ApiResponse> deleteUser(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(new ApiResponse(Instant.now().toString(), "User deleted successfully"));
    }
}
